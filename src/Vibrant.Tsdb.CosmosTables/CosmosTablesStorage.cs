...
namespace Vibrant.Tsdb.CosmosTables
{
     ...
     where TEntry : ICosmosTablesEntry, new()
   {
      ...
      public CosmosTablesStorage( string tableName, string connectionString )
         : this( tableName, connectionString, DefaultThroughputUnits, new ConcurrencyControl( DefaultReadParallelism, DefaultWriteParallelism ), new YearlyPartitioningProvider<TKey>(), new YearlyTableProvider() )
      {
      }
      ...
      private string CreateAfterFilter( TKey key, DateTime from )
      {
         ...
         var toPartitionKey = CosmosTablesKeyCalculator.CalculateMaxPartitionKey( id, key, _partitioningProvider ); // 0000
